...
if (strlen($_SESSION['alogin']) == 0) {
    ...
    if (isset($_POST['update'])) {
        ...
        if (!in_array($extension, $allowed_extensions)) {
            ...
            $sql = "update  tblbooks set bookImage=? where id=?";
            $query = $conn->prepare($sql);
            $query->bind_param('si', $imgnewname, $bookid);
            $query->execute();
            unlink($cpath);
                        ...
                        <div class="panel panel-info">
                            ...
                            <div class="panel-body">
                                <form role="form" method="post" enctype="multipart/form-data">
                                    <?php
                                    $bookid = intval($_GET['bookid']);
                                    $sql = "SELECT tblbooks.BookName,tblbooks.id as bookid,tblbooks.bookImage from  tblbooks  where tblbooks.id=?";
                                    $query = $conn->prepare($sql);
                                    ...
                                    if ($result->num_rows > 0) {
                                        while ($row = $result->fetch_assoc()) { ?>
                                            <input type="hidden" name="curremtimage" value="<?php echo htmlentities($row['bookImage']); ?>">
                                            <div class="col-md-6">
                                            ...
                                            <div class="col-md-6">
                                                <div class="form-group">
                                                    ...
                                                    <input class="form-control" type="text" name="bookname" value="<?php echo htmlentities($row['BookName']); ?>" readonly />
                                                </div>
                                            </div>
